...
namespace Minecraft.CacheRedis
{
    public class RedisHelper : RedisOperatorBase
    {
        ...
        {
            Redis.Set(AddKeyPrefix(key), value.JsonSerialize(), timeSpan);
        }
        public void StringSet<T>(string key, T value) where T : class
        {
            var flag= Redis.Set(AddKeyPrefix(key), value.JsonSerialize());
        }

        public T StringGet<T>(string key) where T : class
        {
            return Redis.Get<string>(AddKeyPrefix(key)).JsonDeserialize<T>();
            //try
            //{
